...
bool g_testSpeedAggr = false;
bool g_testSpeedIncr = false;
bool g_testDiff = false;
bool g_testDiffIncr = false;
bool g_testDiffDist = false;
bool g_testAvalanche = false;
bool g_testBIC = false;
bool g_testCyclic = false;
bool g_testCyclicIncr = false;
bool g_testCyclicAggr = false;
...
HashInfo g_hashes[] =
{
    ...
    { MurmurHash3_x64_128, 128, 0x12BC258E, "Murmur3F", "MurmurHash3 for x64, 128-bit" },

    { MurmurHash3_x64_256, 256, 0x6fee1792, "Murmur3F256", "MurmurHash3 for x64, 256-bit" },
...
void test(hashfunc<hashtype> hash, HashInfo * info)
{
    ...
    if (g_testSpeedAggr || g_testAll) {
        printf("[[[ Speed Tests Aggr ]]]\n\n");
        ...
        for (int i = 1; i < 32; i++) {
            ...
            TinySpeedTestAggr(hashfunc<hashtype>(info->hash), sizeof(hashtype), i, info->verification, true, cycles);
        }

        printf("\n");
    }

    if (g_testSpeedIncr || g_testAll) {
        printf("[[[ Speed Tests Incremental ]]]\n\n");

        BulkSpeedTestIncr(info->verification);
        printf("\n");

        for (int i = 1; i < 32; i++) {
            double cycles;

            TinySpeedTestIncr(sizeof(hashtype), i, info->verification, true, cycles);
        }
    ...
    if (g_testCyclic || g_testAll) {
        ...
        std::cout << "Performs in " << std::chrono::duration_cast<std::chrono::milliseconds>(endTime - startTime).count () << " ms" << std::endl;
    }

    if (g_testCyclicIncr || g_testAll) {
        startTime = std::chrono::high_resolution_clock::now();
        printf("[[[ Keyset Incr 'Cyclic' Tests ]]]\n\n");

        bool result = true;
        bool drawDiagram = false;

        const int keycount = 100000;
        const int keycount1 = 499;

        result &= CyclicKeyTestIncr(sizeof(hashtype) + 0, 8, keycount, keycount1, drawDiagram);
        result &= CyclicKeyTestIncr(sizeof(hashtype) + 1, 8, keycount, keycount1, drawDiagram);
        result &= CyclicKeyTestIncr(sizeof(hashtype) + 2, 8, keycount, keycount1, drawDiagram);
        result &= CyclicKeyTestIncr(sizeof(hashtype) + 3, 8, keycount, keycount1, drawDiagram);
        result &= CyclicKeyTestIncr(sizeof(hashtype) + 4, 8, keycount, keycount1, drawDiagram);
    ...
    if (g_testCyclicAggr || g_testAll) {
        startTime = std::chrono::high_resolution_clock::now();
        printf("[[[ Keyset Aggr 'Cyclic' Tests ]]]\n\n");
        ...
        result &= CyclicKeyTestAggr(sizeof(hashtype) + 0, 8, keycount, keycount1, drawDiagram);
        result &= CyclicKeyTestAggr(sizeof(hashtype) + 1, 8, keycount, keycount1, drawDiagram);
        result &= CyclicKeyTestAggr(sizeof(hashtype) + 2, 8, keycount, keycount1, drawDiagram);
        result &= CyclicKeyTestAggr(sizeof(hashtype) + 3, 8, keycount, keycount1, drawDiagram);
        result &= CyclicKeyTestAggr(sizeof(hashtype) + 4, 8, keycount, keycount1, drawDiagram);
...
void VerifyHash(const void * key, int len, void* seed, void * out)
{
...
int main(int argc, char ** argv)
{
    ...
    //g_testSanity = true;
    //g_testSpeed = true;
    //g_testSpeedAggr = true;
    //g_testSpeedIncr = true;
    //g_testAvalanche = true;
    //g_testBIC = true;
    //g_testCyclic = true;
    g_testCyclicIncr = true;
    //g_testCyclicAggr = true;
    //g_testTwoBytes = true;
    //g_testDiff = true;
    //g_testDiffDist = true;
    //g_testSparse = true;
