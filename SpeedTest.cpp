...
NEVER_INLINE int64_t timehash(const void* key, int len, int seed)
{
    volatile register int64_t begin, end;
    ...
    uint64_t h[4];
    int totalLength = 0;
    MurmurHash3_x64_256Add(key, len, totalLength, h);
    MurmurHash3_x64_256Finalize(h, totalLength, temp);
...
NEVER_INLINE int64_t timehashAdd(const void* key, int len, int seed, int& totalLength, uint64_t* h)
{
    volatile register int64_t begin, end;
...
NEVER_INLINE int64_t timehashFinalize(int& totalLength, uint64_t* h)
{
    volatile register int64_t begin, end;
...
double SpeedTestAggr(pfHash hash, uint32_t seed, const int trials, const int trials1, const int blocksize, const int align)
{
    Rand r(seed);
    ...
    for (int itrial = 0; itrial < trials; itrial++) {
        double t = 0;
        for (int itrial1 = 0; itrial1 < trials1; ++itrial1) {
            r.rand_p(block, blocksize);
...
double SpeedTest(uint32_t seed, const int trials, const int blocksize, const int align)
{
    Rand r(seed);
    ...
    for (int itrial = 0; itrial < trials; itrial++) {
        ...
        double t = (double)timehash(block, blocksize, itrial);
...
double SpeedTestIncr(uint32_t seed, const int trials, const int trials1, const int blocksize, const int align)
{
    Rand r(seed);
    ...
    for (int itrial = 0; itrial < trials; itrial++) {
        ...
        for (int itrial1 = 0; itrial1 < trials1; ++itrial1) {
            r.rand_p(block, blocksize);
            t += (double)timehashAdd(block, blocksize, itrial, totalLength, h);
        }
        t += (double)timehashFinalize(totalLength, h);
        if (t > 0) times.push_back(t);
    }
...
void TinySpeedTestIncr(int hashsize, int keysize, uint32_t seed, bool verbose, double & /*outCycles*/)
{
    ...
    const int trials1 = 999;

    if (verbose) printf("Small key speed test - %4d-byte keys (%d x %d times) - ", keysize, trials, trials1);

    double cycles = SpeedTestIncr(seed, trials, trials1, keysize, 0);
...
void BulkSpeedTestIncr(uint32_t seed)
{
    ...
    for (int align = 0; align < 8; align++) {
        double cycles = SpeedTest(seed, trials, blocksize, align);
...
void TinySpeedTestAggr(pfHash hash, int hashsize, int keysize, uint32_t seed, bool verbose, double & outCycles)
{
    ...
    double cycles = SpeedTestAggr(hash, seed, trials, trials1, keysize, 0);
