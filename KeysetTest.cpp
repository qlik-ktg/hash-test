...
bool VerificationTest(pfHash hash, const int hashbits, uint32_t expected, bool verbose)
{
    ...
    uint64_t seed[4] = { 0 };
    hash(hashes, hashbytes * 256, seed, final);
...
bool CyclicKeyTestIncr(int cycleLen, int cycleReps, const int keycount, const int keycount1, bool drawDiagram)
{
    printf("Keyset Incr 'Cyclic' - %d cycles of %d bytes - (%d x %d) keys\n", cycleReps, cycleLen, keycount, keycount1);
    ...
    for (int i = 0; i < keycount; i++) {
        ...
        for (int j = 0; j < keycount1; ++j) {
            ...
            *(uint32_t*)cycle = f3mix(i ^ 0x746a94f1);
            ...
            MurmurHash3_x64_256Add(key, keyLen, totalLength, &hashes[i]);
        }
        MurmurHash3_x64_256Finalize(&hashes[i], totalLength, &hashes[i]);
    }
    ...
    result &= TestHashList<uint256_t>(hashes, true, true, drawDiagram);
    printf("\n");
...
bool CyclicKeyTestAggr(int cycleLen, int cycleReps, const int keycount, const int keycount1, bool drawDiagram)
{
    printf("Keyset Aggr 'Cyclic' - %d cycles of %d bytes - (%d x %d) keys\n", cycleReps, cycleLen, keycount, keycount1);
    ...
    for (int i = 0; i < keycount; i++) {
        ...
        for (int j = 0; j < keycount1; ++j) {
            ...
            *(uint32_t*)cycle = f3mix(i ^ 0x746a94f1);
            ...
            MurmurHash3_x64_256(key, keyLen, (uint64_t*)(&hashes[i]), &hashes[i]);
        }
    }
